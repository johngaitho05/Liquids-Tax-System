...
class CustomProductPackaging(models.Model):
    ...
    capacity = fields.Float(string='Capacity per Unit')
...
class LiquidTaxSystem(models.Model):
    ...
    def _compute_amount(self, base_amount, price_unit, quantity=1.0, product=None, partner=None,
        ...
        print("packaging is :", product_packaging)
        ...
        if self.amount_type == 'cons_percent' and price_include:
            return base_amount - (base_amount / (1 + self.amount / 100))
        if self.amount_type in ('cons_125', 'cons_300') and not product_packaging:
            raise UserError(_(f"Tax {self.name} applies to packaged products only"))
        if self.amount_type == 'cons_300':
            return product_packaging.qty * product_packaging.capacity * 300
        if self.amount_type == 'cons_125':
            return product_packaging.qty * product_packaging.capacity * 125
    ...
    def compute_all(self, price_unit, currency=None, quantity=1.0, product=None, partner=None, is_refund=False,
                    handle_price_include=True, product_packaging=None):
        print("packaging at compute_all:", product_packaging)
                ...
                if tax.price_include or self._context.get('force_price_include'):
                    if tax.amount_type == 'percent':
                        ...
                        tax_amount = tax._compute_amount(base, sign * price_unit, quantity, product,
                                                         partner, product_packaging) * sum_repartition_factor
        ...
        for tax in taxes:
            ...
            if price_include and total_included_checkpoints.get(i):
                ...
                tax_amount = tax.with_context(force_price_include=False)._compute_amount(
                    base, sign * price_unit, quantity, product, partner, product_packaging)
...
class SaleOrderLine(models.Model):
    ...
    def _compute_amount(self):
        ...
        for line in self:
            ...
            for tax in line.tax_id:
                if tax.amount_type[:4] == 'cons':
                    vals = tax.compute_all(line.price_unit, line.order_id.currency_id, line.product_uom_qty,
                                           ...
                                           product_packaging=line.product_packaging)['taxes']
                    for tax_obj in vals:
                        consumption_tax += tax_obj['amount']

            price_without_vat = line.price_unit + consumption_tax
            ...
            # for tax in line.tax_id:
            #     if tax.amount_type == 'tva':
            #         values = tax.compute_all(reduced_price, line.order_id.currency_id, line.product_uom_qty,
            #                                  product=line.product_id, partner=line.order_id.partner_shipping_id,
            #                                  product_packaging=line.product_packaging)
            #         clean_taxes += values['taxes']
            taxes['taxes'] = clean_taxes

            discount = price_without_vat * (line.discount or 0.0) / 100.0
            prec = line.order_id.currency_id.decimal_places
            base = round(line.price_unit * line.product_uom_qty, prec)
            print("price without vat", price_without_vat)
            print("discount", discount)
            taxes['total_excluded'] = price_without_vat - discount
